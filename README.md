...
<!-------------------------------------
Most Recent Changes:
- fix gain processing, now in [0,100] rather than [0,255] (w/ appropriate readme adjustment)
...
|`gain`         | [%] | Receiver gain that was used when recording the ensemble. |
...
- Pandas functions that access DataFrames can either return a view of the DataFrame or a copy of the DataFrame, but oftentimes it is not clear which return value you will get. For viewing data, getting a view vs. a copy does not make a difference. But for setting data in a DataFrame, there is a big difference: setting data with a view will mutate the original DataFrame and setting data with a copy will mutate the copied DataFrame. This can lead to silent bugs in your code that will give incorrect, and potentially undetectable, results. To help prevent these bugs from occurring, pandas developed a `SettingWithCopyWarning` when you may be in this dangerous situation. The two most common scenarios that will lead to this warning is chained indexing and hidden indexing. Chained indexing can often times be resolved using the .loc access function, and hidden indexing can often be resolved when telling pandas explicitly when you intend to make a copy of the data. More about this warning and the common situations that invoke it can be found in [this article.](https://www.dataquest.io/blog/settingwithcopywarning/)
...
<!------------------------------------>
### MicronSonar TODOs
- check if [error in step size](#note-on-bearing-step-size) is a result of faulty documentation or faulty Seanet Pro software (I suspect this is a result of Seanet Pro software). Make sure that low-level polling software can handle the prescribed scanning resolutions mentioned in the spec sheet. (contact Tritech about this?)
- use pandas categorical data type for ice labels?
- remove ice-slope, only use ice-roughness 
- change "Marginal Ice" to "Partial Ice" (dont want to be confusing about different ice conditions )
...
- add print warnings for when no depth or altitude is given
...
- other plots to generate in the future 
  - waterfall plot of slant-range, bearing, and intensities 

<!------------------------------------>
### README TODOs
- clarify the difference between thick, thin, and marginal ice 
- clarify the difference between smooth and rough ice
- add notes about the available functions and parses available
